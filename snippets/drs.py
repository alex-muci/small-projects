...
def __cts(limits: np.array) -> np.array:
    ...
    for index in range(len(limits)):
        simplex_coords[index][:] = limits                               # same row  
        simplex_coords[index][index] = 0.0
        simplex_coords[index][index] = 1 - simplex_coords[index].sum()  # diagonal changed so rows.sum() ==1
...
def drs_i(ntasks: int, max_utilisation: float, upper_constraints: Optional[Sequence[float]] = None,
    ...
    for count in range(DRS_RETRIES):
        ...
        raise DRSError(
            f'In {DRS_RETRIES} attempts, DRS failed to find a point that converged before floating point error crept in.')
...
if __name__ == "__main__":
    ...
    s = time.time()
    res_unlim = drs(n=10, sumu=0.5)
    print(f"time : {time.time()-s}")

    print(res_unlim,"\n", sum(res_unlim), np.sum(np.absolute(res_unlim)))

    # Umax_i >= U_i > Umin_i > 0, yet it seems to work for Umin <0
    no_tasks = 50
    s = time.time()
    res_lim = drs(n=no_tasks, sumu=.5, upper_constraints=[0.3]*no_tasks, lower_constraints=[-0.3]*no_tasks)
    print(f"time : {time.time()-s}")

    print(res_lim, "\n", sum(res_lim), np.sum(np.absolute(res_lim)), min(res_lim), max(res_lim))


    res_lim_rescaled = res_lim /  np.sum(np.absolute(res_lim))
    print(res_lim_rescaled, "\n", sum(res_lim_rescaled), np.sum(np.absolute(res_lim_rescaled)), min(res_lim_rescaled), max(res_lim_rescaled))
